...
/**
 * Callsign control (Info panel)
 * Renders a small button + label into the Info panel that lets the user
 * view/change/clear their callsign. This only updates localStorage keys
 * (userCallsign / pota_user_callsign) and does **not** touch IndexedDB.
 */
function getStoredCallsign() {
  return (
    localStorage.getItem('userCallsign') ||
    localStorage.getItem('pota_user_callsign') ||
    ''
  );
}

function setStoredCallsign(cs) {
  if (!cs) {
    // Clear both keys when removing
    localStorage.removeItem('userCallsign');
    localStorage.removeItem('pota_user_callsign');
    // Optionally blank in-memory activations so UI stops highlighting
    // until a new callsign is set (IDB is intentionally unchanged):
    try { window.activations = []; } catch {}
  } else {
    const v = String(cs).trim().toUpperCase();
    localStorage.setItem('userCallsign', v);
    localStorage.setItem('pota_user_callsign', v);
  }
  // Refresh any UI that shows the callsign or relies on it
  try { displayCallsign(); } catch {}
  try { applyActivationToggleState(); } catch {}
}

function renderCallsignControl(host) {
  if (!host || host.querySelector('#callsignControl')) return;

  const wrap = document.createElement('div');
  wrap.id = 'callsignControl';
  ...
  val.id = 'callsignControlValue';
  val.style.opacity = '0.85';
  val.textContent = getStoredCallsign() || '—';

  const btn = document.createElement('button');
  btn.type = 'button';
  btn.textContent = 'Set…';
  btn.setAttribute('aria-label', 'Set callsign');
  ...
  btn.addEventListener('click', () => {
    const current = getStoredCallsign();
    const next = prompt('Enter callsign (leave blank to clear):', current || '');
    if (next === null) return; // cancelled
    const trimmed = String(next).trim();
    setStoredCallsign(trimmed);
    // Reflect in control value
    const show = getStoredCallsign();
    val.textContent = show || '—';
  });

  // Optional small clear button on the right
  const clearBtn = document.createElement('button');
  clearBtn.type = 'button';
  clearBtn.textContent = 'Clear';
  clearBtn.setAttribute('aria-label', 'Clear callsign');
  clearBtn.style.border = '1px solid #d0d0d0';
...
/**
 * Finds the Info panel container and mounts the Callsign control.
 * Tries several common selectors and falls back to a MutationObserver
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    mountCallsignControlInInfoPanel();
